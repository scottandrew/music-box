...
class Application {
  ...
  var spectrumDisplay: SpecturmDisplay?
  //var frequencies: [Double] = []
  var arrayManager = AtomicArrayWrapper()

  func start() {
    lv_init()
    let _ = lv_sdl_window_create(800, 600)
    let _ = lv_sdl_mouse_create()
    let _ = lv_sdl_mousewheel_create()
    let _ = lv_sdl_keyboard_create()
    ...
    spectrumDisplay = SpecturmDisplay(
      parent: screen!, width: screen!.coordinates.width, height: 100)
    spectrumDisplay?.setPosition(x: 0, y: 310)

    analyzer = SpectrumAnalyzer(sampleRate: 44100.0, framesPerBuffer: 512)
  ...
  func run() {
    while true {
      ...
      spectrumDisplay?.updateFrequencies(
        frequencies: arrayManager.getArray(), startIndex: analyzer!.startIndex,
        spectroSize: analyzer!.spectrumSize)
    }
  }

  func startAudioCapture() {
    let error = audio.startup()
    ...
    let inputParameters = PaStreamParameters(
      ...
      suggestedLatency: devices[1].defaultLowInputLatency,
      hostApiSpecificStreamInfo: nil)

    stream = AudioStream(
      inputParameters: inputParameters, outputParameters: nil, callback: processAudioFrames)
  ...
  func processAudioFrames(
    ...
    let frequencies = analyzer?.calculate(input: frames, frameCount: Int(framesPerBuffer))
    ...
    arrayManager.setArray(frequencies ?? [])
